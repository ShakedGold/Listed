...
async function getPosts(sortBy) {
	...
	if(sort.By === 'Top'){
		...
		q = query(postsRef, where('created', '>', timeRangeDate[sort.Of]), where('lists', 'array-contains', route.params.name)).withConverter(postConverter);
		const querySnapshot = await getDocs(q);
		return querySnapshot.docs.sort((a, b) => b.data().votes - a.data().votes).map((doc) => doc.data());
	}
	q = query(postsRef, orderBy(translation[sort.By], 'desc'), where('lists', 'array-contains', route.params.name)).withConverter(postConverter);
	const querySnapshot = await getDocs(q);
